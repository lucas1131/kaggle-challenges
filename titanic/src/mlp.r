...
mlp.forward <- function(mlp, input){
	...
	# fwd$f.hidden = as.gvector(rep.int(0, mlp$size$hidden), type="double")
	# fwd$df.hidden = as.gvector(rep.int(0, mlp$size$hidden), type="double")
	# fwd$f.output = as.gvector(rep(0, mlp$size$output), type="double")
	# fwd$df.output = as.gvector(rep(0, mlp$size$output), type="double")

	# net.hidden = as.gvector(rep(0, mlp$size$hidden), type="double")
	# net.output = as.gvector(rep(0, mlp$size$output), type="double")
	...
	tmp = c(input, 1)
	# tmp2 = as.gvector(tmp, type="double")
...
mlp.train <- function(mlp, train.input, train.output, step=0.1, threshold=1e-2){
	...
	# g(mlp$layers$hidden, type="double", dup=FALSE)
	# g(train.input, type="double", dup=FALSE)
	...
	while(error > threshold){
		...
		for(i in 1:train.input.size){
			...
			# delta.output = as.gvector(delta * fwd$df.output, type="double", dup=FALSE)
			...
			# mlp$layers$output = mlp$layers$output + 
			# 	step*(tcrossprod(delta.output, c(as.gvector(fwd$f.hidden, type="double", dup=FALSE), 1)))
			...
			# mlp$layers$hidden = mlp$layers$hidden +
			# 	step*(tcrossprod(as.gvector(delta.hidden, type="double", dup=FALSE), c(train.input[i,], 1)))
		}
